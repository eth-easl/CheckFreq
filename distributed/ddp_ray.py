...
print(config_model.get_model_by_name)

spec1 = importlib.util.spec_from_file_location("module.name", "/datadrive/home/ubuntu/CheckFreq/distributed/cf_checkpoint.py")
cf_checkpoint = importlib.util.module_from_spec(spec1)
spec1.loader.exec_module(cf_checkpoint)
CFCheckpoint = cf_checkpoint.CFCheckpoint


spec2 = importlib.util.spec_from_file_location("module.name", "/datadrive/home/ubuntu/CheckFreq/distributed/cf_manager.py")
cf_manager = importlib.util.module_from_spec(spec2)
spec2.loader.exec_module(cf_manager)
CFManager = cf_manager.CFManager
CFMode = cf_manager.CFMode

spec3 = importlib.util.spec_from_file_location("module.name", "/datadrive/home/ubuntu/CheckFreq/distributed/cf_iterator.py")
cf_iterator = importlib.util.module_from_spec(spec3)
spec3.loader.exec_module(cf_iterator)
CFIterator = cf_iterator.CFIterator
...
def main():
    ...
    if args.distributed:
        print("------- distributed")
        args.gpu = train.local_rank() % torch.cuda.device_count()
        torch.cuda.set_device(args.gpu)
        args.world_size = torch.distributed.get_world_size()

    args.total_batch_size = args.world_size * args.batch_size
    print("--------- world size: ", args.world_size)
    print("--------- batch size: ", args.total_batch_size)
    print("--------- cf.iterator: ", args.cf_iterator)
    #if args.fp16:
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        avg_train_time = train_epoch(train_loader, model, criterion, optimizer, epoch, df, cf_manager)
...
def train_epoch(train_loader, model, criterion, optimizer, epoch, df, cf_manager):
