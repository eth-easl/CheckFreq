...
spec1 = importlib.util.spec_from_file_location("module.name", "/datadrive/home/ubuntu/CheckFreq/distributed/cf_manager.py")
cf_manager = importlib.util.module_from_spec(spec1)
spec1.loader.exec_module(cf_manager)
CFMode = cf_manager.CFMode

#from disk_bw import get_storage_bandwidth
spec2 = importlib.util.spec_from_file_location("module.name", "/datadrive/home/ubuntu/CheckFreq/distributed/disk_bw.py")
get_storage_bandwidth = importlib.util.module_from_spec(spec2)
spec2.loader.exec_module(get_storage_bandwidth)
...
class CFIterator:
	def __init__(
		...
		# This is temporary
		self.chk_fn = getattr(self._cf_manager, "save_cpu")
		print(self.chk_fn) 
	...
	def __next__(self):
			...
			if self._chk_mode == CFMode.MANUAL:
				if self.at_gpu:
					if self.noop:    
						...
						self._cf_manager.save(synchronous=False, additional_snapshot=self.state_dict(), persist=self._persist, use_thread=False)
				else:
					if self.noop:
		...
		try:
			...
			if self._worker_id == 0 and self._chk_mode == CFMode.MANUAL:
				...
				print("------------- self.chk_fn: ", self._chk_fn)
				self._chk_fn(
					additional_snapshot=self.state_dict(),
					is_epoch = True,
					epoch = self._epoch) 
